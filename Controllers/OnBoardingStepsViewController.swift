...
class OnBoardingStepsViewController: UIViewController {
    ...
    @objc private func handleNextButtonTap() {
        if currentStep < imageArr.count - 1 {
            ...
            // Send a user to Login & Signup storyboard.
            let loginStoryboard = UIStoryboard(name: "SignupAndLogin", bundle: nil)
    ...
    func buttonStyle() {
        ...
        OnBoardNextBtn.titleLabel?.font = UIFont(name: FX.fonts.fontBold, size: 20.0)
    ...
    private func applyGradientToButton() {
        ...
        let color1 = UIColor(hex: FX.colors.color1) ?? .blue
        let color2 = UIColor(hex: FX.colors.color2) ?? .blue
